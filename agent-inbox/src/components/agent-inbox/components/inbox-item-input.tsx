    ...
    <div className="flex flex-col gap-4 items-start w-full p-6 rounded-lg border-[1px] border-gray-300">
      ...
      {Object.entries(editResponse.args.args).map(([k, v], idx) => {
        ...
        if (
          ...
          const strValue = typeof v === "string" ? v : (v != null ? JSON.stringify(v) : "");
          defaultRows.current[k as keyof typeof defaultRows.current] = !strValue.length
            ? 3
            : Math.max(strValue.length / 30, 7);
        }
        const numRows =
